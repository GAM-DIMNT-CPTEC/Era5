...
!tar end 
  ...
  REAL(KIND=r8)     , PUBLIC           :: gbaco2=0.0_r8   
  ...
  INTEGER           , PUBLIC           :: thermcell=0  ! 1 pbl Mellor Yamada 2.0
...
! 
  ...
  INTEGER , PUBLIC                     :: ndhndrct=86   ! nedrct : unit file number to output description of dhn prognostics
  INTEGER , PUBLIC                     :: ndhnfcst=59     ! nefcst : unit file number to output files list of dhn prognostics
  ...
  CHARACTER(LEN=10 ), PUBLIC           :: labelsi_flxco2
  CHARACTER(LEN=10 ), PUBLIC           :: labelsj_flxco2
...
CONTAINS
  ...
  SUBROUTINE ReadNameList()
    ...
    NAMELIST /PHYSPROC/UNIFIED,iswrad,ilwrad,iccon,ilcon,iqdif, &
         ...
         grepar1,grepar2,grepar3,grepar4,iglsm_w,thermcell,sfcpbl,atmpbl,&
         PBLEntrain,schemes,isimveg,isimco2,gbaco2,OCFLUX,SLABOCEAN,ICEMODEL,omlmodel,oml_hml0,&
    ...
    NAMELIST /COMCON/initlz , nstep  , fint   , intsst ,intndvi,intsoilm,intflxco2, ndord  ,&
         filta  , percut , varcut , ifsst  ,ifndvi, &
         ifsnw  , ifalb  , ifslm  ,ifslmSib2, allghf , &
         ifco2  ,ifco2flx, ifozone  ,iftracer, ifaeros, &
...
!    IF(microphys)ilcon='MIC'
  ...
  SUBROUTINE DumpOptions()
    ...
    CALL MsgOne(h, " input CO2 file name is "//TRIM(fNameCO2FLX))
...
!    fNameCldOptLW
  ...
  SUBROUTINE setsst (c0)
    ...
    labelsi_soilm=labelsi
    labelsj_soilm=labelsj
    labelsi_flxco2=labelsi
    labelsj_flxco2=labelsj
...
!_____________________________________________________________________    
    ...
    IF ( TRIM(NMCO2FX) == 'co2flxwkl' ) THEN
       ...
       END IF
    ELSE IF ( TRIM(NMCO2FX) == 'co2flxwkd' ) THEN
    ...
    ELSE IF ( TRIM(NMCO2FX) == 'co2flxanp' ) THEN
       fNameCO2FLX=TRIM(path_in1)//'FLUXCO2Clima'//LABELS//'.G'//TRIM(c0)
    ELSE
       NMCO2FX='co2flxaoi'
       fNameCO2FLX=TRIM(path_in1)//'FLUXCO2Clima'//LABELS//'.G'//TRIM(c0)
...
!tar begin
!SET COARSE CLIMATE AEROSOL AOD,ASY,SSA,AODvrt file names

    fNameClimAodRRTM = TRIM(path_in1)//'ClimAodRRTM'//'.G'//c0         !ClimAodRRTM.G00096

    fNameClimAsyRRTM = TRIM(path_in1)//'ClimAsyRRTM'//'.G'//c0         !ClimAsyRRTM.G00096

    fNameClimSsaRRTM = TRIM(path_in1)//'ClimSsaRRTM'//'.G'//c0         !ClimSsaRRTM.G00096

    fNameClimAodVrt = TRIM(path_in1)//'ClimAodVrt'//'.G'//c0           !ClimAodVrt.G00096

!tar end
!
!tar begin
!SET Fine 2000 CLIMATE AEROSOL AOD,ASY,SSA,AODvrt file names

    fNameFineAodRRTM = TRIM(path_in1)//'F2000AodRRTM'//'.G'//c0         !F2000AodRRTM.G00096

    fNameFineAsyRRTM = TRIM(path_in1)//'F2000AsyRRTM'//'.G'//c0         !F2000AsyRRTM.G00096

    fNameFineSsaRRTM = TRIM(path_in1)//'F2000SsaRRTM'//'.G'//c0         !F2000SsaRRTM.G00096

    fNameFineAodVrt = TRIM(path_in1)//'F2000AodVrt'//'.G'//c0           !F2000AodVrt.G00096
