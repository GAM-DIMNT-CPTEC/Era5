...
CONTAINS
  ...
  SUBROUTINE RunCloudOpticalProperty2(       &
    ...
    ! Atmospheric fields
    REAL(KIND=r8),    INTENT(in   ) :: prsii  (IM,LM+1)
    REAL(KIND=r8),    INTENT(in   ) :: prsll  (IM,LM)
    REAL(KIND=r8),    INTENT(in   ) :: phii  (IM,LM+1)
    REAL(KIND=r8),    INTENT(in   ) :: phil  (IM,LM)
    ...
    REAL(KIND=r8)   , INTENT(IN   ) :: EFFCS (IM,LM) 
    REAL(KIND=r8)   , INTENT(IN   ) :: EFFIS (IM,LM) 
    ! Cloud properties
    REAL(KIND=r8), INTENT(inout) :: CldLow (IM)
    REAL(KIND=r8), INTENT(inout) :: CldMed (IM)
    REAL(KIND=r8), INTENT(inout) :: CldHgh (IM)
    ...
    REAL (kind=r8)   :: Psurf (IM)
    REAL (kind=r8)   :: DeltaP (IM,LM) 
...
!    REAL(KIND=r8) :: cwp    (IM,LM)! in-cloud cloud (total) water path
    REAL(KIND=r8) :: cldaux    (IM,LM)     ! cloud water scale heigh (m)
    ...
    DO k=1,LM
       DO i = 1,IM
         DeltaP(i,k) = ((prsii(i,k)) - (prsii(i,k+1)))/prsii(i,1)
       END DO
    END DO

    DO i=1,IM
       Psurf(i) =prsii  (i,1)/100.0_r8
    END DO
    ...
    DO i = 1, IM
      ! prsi(i,LM + 1)=MAX(ps(i)*si(LM+1)/10.0_r8,1.0e-12_r8) !mb  -- > cb
       prsi(i,LM + 1)= prsii(i,LM + 1)/1000.0_r8 !Pa  -- > cb
    END DO

    DO k = 1, LM
       DO i = 1, IM
          rrime(i,k)= 1.0_r8
          prsl(i,k)=prsii(i,k)/1000.0_r8!ps(i)*si(k)/10.0_r8 !Pa  -- > cb
          prsi(i,k)=prsii(i,k)/1000.0_r8!ps(i)*si(k)/10.0_r8 !Pa  -- > cb
       ENDDO
    ENDDO
    ...
    DO k = 1, LM
       DO i = 1, IM
          es  = MIN( prsl(i,k), 0.001_r8 * fpvs2es5( tgrs(i,k) ) )   ! fpvs in pa
    ...
    DO k=1,LM
       DO i = 1,IM
          pw(i) = pw(i) + DeltaP(i,k)*(qgrs(i,k))
       END DO
    END DO
    DO i = 1,IM
       pw(i)=100.0_r8*pw(i)*Psurf(i)/con_g
    END DO
    !
    ...
    DO k=1,LM
       DO i=1,IM
          lmixr(i,k)=clwp(i,k)*con_g*1.0e-5_r8/DeltaP(i,k)/Psurf(i)
       END DO
    END DO
   ...
   DO k=1,LM
      DO i=1,IM
         cldaux(i,k) =MAX(cld(i,LM-k+1),clu(i,LM-k+1))
        ! clouds(i,k,1) =MAX(clouds(i,k,1) ,cldaux(i,k))
         clouds(i,k,1) =(clouds(i,k,1) +cldaux(i,k))/2.0_r8
         clouds(i,k,1) =MAX(clouds(i,k,1) , 2.0e-32_r8)

         !clouds(i,k,3)=MAX(clouds(i,k,3),EFFCS (i,k)) !     rel(i,k)= clouds(:,:,3) - mean eff radius for liq cloud      (micron)      !
         !clouds(i,k,5)=MAX(clouds(i,k,5),EFFIS (i,k)) !     rei(i,k)= clouds(:,:,5) - mean eff radius for ice cloud      (micron)      !

         clouds(i,k,3)=(clouds(i,k,3)+EFFCS (i,k))/2.0_r8 !     rel(i,k)= clouds(:,:,3) - mean eff radius for liq cloud      (micron)      !
         clouds(i,k,5)=(clouds(i,k,5)+EFFIS (i,k))/2.0_r8 !     rei(i,k)= clouds(:,:,5) - mean eff radius for ice cloud      (micron)      !
    ...
    DO k=1,LM
       DO i=1,IM
          ...
          IF ( TRIM(ILCON) .EQ. 'MORR') THEN
             ...
             lmixr(i,k)=clwp(i,k)*con_g*1.0e-5_r8/DeltaP(i,k)/Psurf(i) ! ! Ice/Water mixing ratio
             ...
             !taud(i,k) =  MAX(MIN(MAX(kabs*clwp(i,k), absliq1* clouds(i,k,2) + absrain * clouds(i,k,6) + abssnow0 * clouds(i,k,8)),500.0_r8),1.0e-8_r8)
             taud(i,k) = MAX(MIN( kabs*clwp(i,k) + absrain * clouds(i,k,6) + abssnow0 * clouds(i,k,8),500.0_r8),0.025_r8)
...
!    REAL(KIND=r8) :: cwp    (IM,LM)! in-cloud cloud (total) water path
    ...
    DO k = 1, LM
       DO i = 1, IM
          rrime(i,k)= 1.0_r8
          prsl(i,k)=prsll(i,k)/1000.0_r8!ps(i)*si(k)/10.0_r8 !Pa  -- > cb
          prsi(i,k)=prsii(i,k)/1000.0_r8!ps(i)*si(k)/10.0_r8 !Pa  -- > cb
       ENDDO
    ENDDO
    ...
    DO k = 1, LM
       DO i = 1, IM
          ...
          !ELSE
             ...
             qs  = MAX( QMIN, con_eps * es /MAX(( (prsl(i,k)         + con_epsm1*es)    ),1.0e-21_r8))
          !END IF
    ...
    DO k=1,LM
       DO i=1,IM
          ...
          IF ( TRIM(ILCON) .EQ. 'MORR' .OR.TRIM(ILCON).EQ.'HUMO'.OR. TRIM(ILCON).EQ.'HWRF' ) THEN
             ...
             ! cloud optical depth
             !taud(i,k) = kabs*clwp(i,k)! g/m2
             taud(i,k) =  MAX(MIN(MAX(kabs*clwp(i,k), absliq1* clouds(i,k,2) + absrain * clouds(i,k,6) + abssnow0 * clouds(i,k,8)),500.0_r8),0.025_r8)
...
!!!!        rei(i,k)   = max(30.0, min(rei(i,k), 300.0))
  ...
  SUBROUTINE Cloud_Micro_WRF(&
    ...
    ! Atmospheric Fields
    REAL(KIND=r8),    INTENT(in   ) :: prsii  (ncols,kMax+1)
    REAL(KIND=r8),    INTENT(in   ) :: prsll  (ncols,kMax)
...
!    REAL(KIND=r8) :: weight
    ...
    DO i = 1, nCols
       ...
       prsi(i,kMax + 1)=prsii(i,kMax + 1)/1000.0_r8 !Pa  -- > cb

    END DO

    DO k = 1, kMax
       DO i = 1, nCols
          prsl(i,k)=prsll(i,k)/1000.0_r8!Pa  -- > cb !ps(i)*si(k)/10.0_r8 ! cb
          prsi(i,k)=prsii(i,k)/1000.0_r8!Pa  -- > cb !ps(i)*si(k)/10.0_r8 ! cb
       ENDDO
    ENDDO
