...
CONTAINS
  ...
  SUBROUTINE RunMicro_LrgScl(geshem, tf, qs, qf,prsi,prsl, dtdt,dqdt, dt, &
    ...
    DO k=1,kMax
       DO i=1,ncols
          ...
          IF( prsl(i,k)/1000.0_r8 + const2*qs(i,k) == 0.0_r8)THEN  
              qs(i,k)   = const1*qs(i,k)/MAX(prsl(i,k)/1000.0_r8 + const2*qs(i,k),1.0e-12_r8)
          ELSE
              qs(i,k)   = const1*qs(i,k)/MAX(prsl(i,k)/1000.0_r8 + const2*qs(i,k),1.0e-12_r8)
          END IF
          !dpovg(i,k)= (del(k)*rgrav)*ps(i)
          dpovg(i,k)= (DeltaP(i,k)*rgrav)*(prsl(i,1)/1000.0_r8)
       END DO
    END DO
    ...
    DO k=1, kMax
       l= kMax +1-k
       DO i=1, ncols
          dtdt (i,l)= (tf(i,l) - t(i,l) )/dt
          dqdt (i,l)= (qf(i,l) - q(i,l) )/dt
       END DO
    END DO
