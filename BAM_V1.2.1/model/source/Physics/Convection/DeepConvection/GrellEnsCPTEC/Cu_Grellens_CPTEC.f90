...
CONTAINS
  ...
  SUBROUTINE RunCu_GrellEnsCPTEC(&
       prsi   ,prsl   ,ua     ,va     ,omg    ,t2     ,tn1    , &
       q2     ,qn1    ,zz     ,xland  ,dtime  ,RAINCV , &
       kuo    ,ktop   ,kbot   ,plcl   ,qliq   ,nCols  ,kMax,dtdt ,dqdt )
...
!    REAL(KIND=r8)                   :: down_massflx(nCols)
   ...
   INTEGER                          :: ierr     (nCols)
    ...
    DO i=1,nCols
       ...
       plcl(i) = 1.0e0_r8
       !psur (i) = ps(i)*10.0_r8
       psur (i) = prsi  (i,1)/100.0_r8 ! convert Pa to mb 
    END DO
    !
...
!    ter11(1:nCols) = zz(1:nCols)
    DO k = 1, kMax
       DO i = 1,nCols
          ...
          po  (i,k) = prsl(i,k)/100.0_r8                ! pressure in mbar
          p   (i,k) = po(i,k)
          us  (i,k) = ua(i,k) 
          vs  (i,k) = va(i,k) 
          omeg(i,k) = omg(i,k)
