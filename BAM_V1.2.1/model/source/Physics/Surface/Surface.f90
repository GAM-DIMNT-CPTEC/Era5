...
MODULE Surface
  ...
  REAL(KIND=r8), TARGET, ALLOCATABLE :: VELC100m   (:,:)
  REAL(KIND=r8), TARGET, ALLOCATABLE :: DirWind100m(:,:)
...
CONTAINS
  SUBROUTINE InitSurface(ibMax             ,jbMax              ,iMax          ,jMax          , &
    ...
    ALLOCATE(VELC100m   (ibMax,jbMax)); VELC100m=0.0_r8
    sfc%VELC100m=>VELC100m(1:ibMax,1:jbMax)
    ALLOCATE(DirWind100m(ibMax,jbMax)); DirWind100m=0.0_r8
    sfc%DirWind100m=>DirWind100m(1:ibMax,1:jbMax)

    RESTART_SURFACE=RESTART
    IF(initlz == 0 .or. initlz == -1 .or. initlz == -2 .or. initlz == -3 )RESTART_SURFACE=.TRUE.

    CALL Init_Sfc_SeaFlux_Interface(ibMax,jbMax,kMax,ibMaxPerJB,RESTART_SURFACE,fNameSlabOcen,fgtmp)
...
!add solange
    ...
    REAL(KIND=r8) :: VELC100m_local   (nCols)
    REAL(KIND=r8) :: DirWind100m_local(nCols)
...
!    rbyg=gasr/grav*delsig(1)*0.5e0_r8   !  m / K
    ...
    CALL CALC2MLAND(nmax,nmax,dtc3x,&
         ...
         ztn  (1:nmax),z0x(1:nmax),rhoair(1:nmax),hr(1:nmax),ta(1:nmax),ea(1:nmax),&
         etc(1:nmax),etg(1:nmax),psur(1:nmax),d(1:nmax) ,znew,qsurfl(1:nmax),zl1(1:nmax),&
         theta100m(1:nmax),VELC100m_local(1:nmax),q100m(1:nmax),zland(1:nmax))
...
!PK    END IF
    ...
    DO i=1,nCols
       ...
       IF(imask(i).GE.1_i8) THEN
          ncount=ncount+1
          VELC_100M(i)=VELC100m_local(ncount)!theta2m(1:nmax),velc2m(1:nmax),q2m(1:nmax)
    ...
    DO i=1,nCols
       ...
       VELC100m_local(i)     = VELC_100M(i)
       VELC100m   (i,latitu) = VELC100m_local(i)
       DirWind100m_local(i)  = 180.0_r8*atan2((gu (i,2)/SIN( colrad(i))),(gv (i,2)/SIN( colrad(i))))/3.1415_r8 
       IF(DirWind100m_local(i) < 0.0_r8)DirWind100m_local(i)=DirWind100m_local(i)+360.0_r8
       DirWind100m(i,latitu)=DirWind100m_local(i)
    END DO
    !
    ...
    IF( StartStorDiag)THEN
       CALL SfcDiagnStorage(&
            ...
            VELC50m (1:nCols)          ,DirWind50m(1:nCols)      ,VELC100m_local(1:nCols)        ,DirWind100m_local(1:nCols)      , &
  ...
  SUBROUTINE SfcDiagnStorage(&
            ...
            VELC100m_local,DirWind100m_local,imask    ,hc        ,hg       ,ect        , &
    ...
    REAL(KIND=r8), INTENT(IN) :: VELC100m_local  (nCols)
    REAL(KIND=r8), INTENT(IN) :: DirWind100m_local(nCols)
    ...
    IF( (kt.NE.0) .OR. (jdt.NE.1) ) THEN
       ...
       IF(dodia(nDiag_sp100m)) CALL updia(VELC100m_local   ,nDiag_sp100m,latitu)
       ...
       IF(dodia(nDiag_dr100m)) CALL updia(DirWind100m_local,nDiag_dr100m,latitu)
