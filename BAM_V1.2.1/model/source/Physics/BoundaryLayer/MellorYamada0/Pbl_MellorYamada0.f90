...
CONTAINS
  ...
  SUBROUTINE InitPbl_MellorYamada0()
    REAL(KIND=r8)     :: gam1
    REAL(KIND=r8)     :: gam2
  ...
  SUBROUTINE MellorYamada0 ( &
    ...
    REAL(KIND=r8),    INTENT(in   ) :: qmsfc  (ncols,kmax,5+nClass+nAeros)
    ...
    DO i=1,ncols
      ...
      qmtx   (i,1,1:5+nClass+nAeros) = qmsfc  (i,1,1:5+nClass+nAeros)
  ...
  SUBROUTINE ympbl0 ( &
    ...
    REAL(KIND=r8)    :: PK_a0(ncols,kmax)
    REAL(KIND=r8)    :: PK_b0(ncols,kmax)
    REAL(KIND=r8)    :: PK_t0(ncols,kmax)
    REAL(KIND=r8)    :: PK_t1(ncols,kmax)
    REAL(KIND=r8)    :: PK_sigr  (ncols,kmax)
    REAL(KIND=r8)    :: PK_sigriv(ncols,kmax)
    REAL(KIND=r8)    :: PK_con0  (ncols,kmax)
    REAL(KIND=r8)    :: PK_con1  (ncols,kmax)
    REAL(KIND=r8)    :: PK_con2  (ncols,kmax)
    ...
    IF(PBL == 1)THEN
       ...
       DO k = 1,kMax 
          DO i=1,nCols
           ...
           PK_con0  (i,k)=gasr*((prsi(i,k)/prsi(i,1)) - (prsi(i,k+1)/prsi(i,1)))/(grav*(prsl(i,k)/prsi(i,1)))
         END DO
      END DO
       ...
       DO k = 1, kmax-1
          DO i=1,nCols
           ...
           PK_con1  (i,k) = grav*(prsi(i,k+1)/prsi(i,1))/(gasr*(((prsl(i,k)/prsi(i,1)) - (prsl(i,k+1)/prsi(i,1)))))
...
!          t0    (   k)=(sig(k+1)-sigml(k+1))/(sig(k+1)-sig(k))
           ...
           PK_t0    (i,   k)=((prsl(i,k+1)/prsi(i,1)) - (prsi  (i,k+1)/prsi  (i,1))) / &
                          ((prsl(i,k+1)/prsi(i,1)) - (prsl(i,k)/prsi(i,1)))


 !          t1    (   k)=(sigml(k+1)-sig(k  ))/(sig(k+1)-sig(k))
           PK_t1    (i,   k)=((prsi  (i,k+1)/prsi  (i,1)) - (prsl(i,k)/prsi(i,1)))/&
                        ((prsl  (i,k+1)/prsi  (i,1)) - (prsl(i,k)/prsi(i,1)))
         !   sigk  (   k)=       sl(k)**akappa

          !sigr  (   k)=sigk(k)*sigkiv(k+1)
           PK_sigr  (i,k)= ((prsl(i,k)/prsi(i,1))**akappa)*  (1.0_r8/((prsl(i,k+1)/prsi(i,1))**akappa))
          !sigriv( k+1)=sigk(k+1)*sigkiv(k)
           PK_sigriv(i,k+1)= ((prsl(i,k+1)/prsi(i,1))**akappa )  *  (1.0_r8/((prsl(i,k)/prsi(i,1))**akappa))
          ...
          PK_a0    (i,k   )=gbyr*((prsi  (i,k+1)/prsi  (i,1))**2)/(((prsi(i,k)/prsi(i,1)) - (prsi(i,k+1)/prsi(i,1)))*((prsl(i,k)/prsi(i,1)) - (prsl(i,k+1)/prsi(i,1))))

          !bb0    (i,k+1 )=gbyr*sigml(k+1)**2/(delsig(k+1)*(sig(k)-sig(k+1)))

          PK_b0    (i,k+1 )=gbyr*((prsi  (i,k+1)/prsi  (i,1))**2)/(((prsi(i,k+1)/prsi(i,1)) - (prsi(i,k+2)/prsi(i,1)))*((prsl(i,k)/prsi(i,1)) - (prsl(i,k+1)/prsi(i,1))))
       ...
       DO k = 1, kmax-1
          DO i = 1, ncols
             Pbl_ATemp(i,k)  = PK_t0(i,k)*gt(i,k) + PK_t1(i,k)*gt(i,k+1)
          END DO
       END DO
       ...
       DO k = 1, kmax-1
          DO i = 1, ncols
             ...
             !               SIN(colrad(i))**2
             !
             Pbl_Shear(i,k)=(PK_con1(i,k)*csqiv(i))*Pbl_ITemp(i,k)*((gu(i,k)-gu(i,k+1))**2+(gv(i,k)-gv(i,k+1))**2)
       ...
       k=1
       DO i = 1, ncols
          Pbl_HgtLyI(i,k)=PK_con0(i,k)*gt(i,k)
       END DO

       DO k = 2, kmax
          DO i = 1, ncols
             Pbl_HgtLyI(i,k)=Pbl_HgtLyI(i,k-1)+PK_con0(i,k)*gt(i,k)
          END DO
       END DO
...
!             tkemyj(i,k)=Pbl_EddEner(i,K)**2 
       ...
       ! 
       CALL VDIFT(kMax,nCols,twodti,PK_sigriv,PK_sigr,Pbl_ITemp,Pbl_CoefKh,a,b,gt,gmt)
    ...
    ELSE
       ...
       DO k = 1, kmax
          DO i=1,ncols
             pmid(i,k) = prsl  (i,k)!gps(i)*pbl_sl(k)*100.0_r8
          END DO
       END DO
       !
...
!    END DO
       ...
       DO i=1,ncols
          ...
          thvsrf(i) = (tsfc0(i)*(1.0_r8/((prsl(i,1)/prsi(i,1))**akappa)))*(1.0_r8 + 0.61_r8*qsfc0(i))
          !
          !
