MODULE PhysicalFunctions
  ...
  PUBLIC :: K_index
  PUBLIC :: TT_index
  PUBLIC :: calc_poda_index,liftix_index

CONTAINS

  SUBROUTINE InitPhysicalFunctions()
    IMPLICIT NONE
    !
  ...
  SUBROUTINE INDEXLEVEL(prsl,iMax,kMax,k850,k700,k500)
    IMPLICIT NONE
    INTEGER      , INTENT(IN   ) :: iMax
    INTEGER      , INTENT(IN   ) :: kMax
    REAL(KIND=r8), INTENT(IN   ) :: prsl(iMax,kMax)!Pa
    INTEGER      , INTENT(OUT  ) :: k850(iMax),k700(iMax),k500(iMax)
    REAL(KIND=r8) :: Dsl  (iMax,kMax)
    REAL(KIND=r8) :: DslPa  (kMax)
    INTEGER       :: level(iMax,1)
    INTEGER       :: k,i
    Dsl=1.0e20_r8
    DO k=1,kMax
       DO i=1,iMax
          Dsl(i,k)=abs(prsl(i,k) - 85000.0_r8)
       END DO
    END DO
    DO i=1,iMax 
       DslPa=Dsl(i,1:kMax)
       level(i,1)=MINLOC(DslPa,DIM=1)
       k850(i)=level(i,1)
    END DO
    ...
    DO k=1,kMax
       DO i=1,iMax
          Dsl(i,k)=abs(prsl(i,k) - 50000.0_r8)
       END DO
    END DO
    DO i=1,iMax
       DslPa=Dsl(i,1:kMax)
    ...
    DO k=1,kMax
       DO i=1,iMax
          Dsl(i,k)=abs(prsl(i,k) - 70000.0_r8)
       END DO
    END DO
    DO i=1,iMax
       DslPa=Dsl(i,1:kMax)
  ...
  SUBROUTINE stma(the,pk,tma,qma)
    ...
    tma=0.0_r8;qma=0.0_r8
    xj=0.0_r8;yj=0.0_r8;ftx1=0.0_r8;ftx2=0.0_r8;qx1=0.0_r8;qx2=0.0_r8
    ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  FUNCTION calc_poda_index(t,c,p,z) result(a)
    IMPLICIT NONE
    REAL(KIND=r8), INTENT(IN)  :: t(:) ! t= level-  temp in K
    REAL(KIND=r8), INTENT(IN)  :: c(:) ! c= 2m-  temp in K
    REAL(KIND=r8), INTENT(IN)  :: p(:) ! p= sf-  Pres in hPa
    REAL(KIND=r8), INTENT(IN)  :: z(:) ! z= sf-  topo in m

    REAL(KIND=r8)    :: x,e
    REAL(KIND=r8)    :: pv(Size(c,1))
    REAL(KIND=r8)    :: f (Size(c,1)) ! f= sl pressure in hPa
    REAL(KIND=r8)    :: g (Size(c,1)) ! g= 2m- dew point T in K
    REAL(KIND=r8)    :: a (Size(c,1)) ! a=   Health Indexes poda index (g/m3)
    REAL(KIND=r8)    :: slp (Size(c,1)) 
    INTEGER :: i,j
    DO i=1,Size(c,1)
       slp(i)=p(i)*((1.0_r8  - (0.0065_r8*max(z(i),0.0_r8)/(t(i)+0.0065*max(z(i),0.0_r8))))**(-5.257_r8))
    END DO

    DO i=1,Size(c,1)
       !Real(r8) saturation vapor pressure in Pascals [Pa]
       pv(i)=fpvs2es5(c(i))
       !Real(r8) saturation vapor pressure in Milibar [hPa]
       f(i)= slp(i)
       g(i) = ftdpx(pv(i))
    END DO
   !g=DewPoint(pv,nCols)
   ...
   DO i=1,Size(c,1)
      g(i)=g(i)-273.15_r8
      x= 235.0_r8*log(f(i)/1.333224_r8) - 1936.4_r8 - g(i)*25.34_r8 + &
      g(i)*log(f(i)/1.333224_r8)

      e=exp( - x/(g(i)+235.0_r8) + log(f(i)/1.333224_r8) - log(622.0_r8) )

      a(i)=80.51_r8 * f(i)/c(i) * (1.0_r8 - e/f(i))
      a(i)=max(0.0_r8,a(i))
   ENDDO
   
   RETURN
  END FUNCTION calc_poda_index
  !-------------------------------------------------------------------------------
  !
  ...
  SUBROUTINE calc_cape( &
    ...
    INTEGER, INTENT(IN   ) :: kMax
    REAL(KIND=r8), INTENT(IN   ) :: prsi (nCols,kMax+1)!REAL(KIND=r8), INTENT(IN   ) :: prsi(iMax,kMax) 
    REAL(KIND=r8), INTENT(IN   ) :: prsl (nCols,kMax  )!REAL(KIND=r8), INTENT(IN   ) :: prsl(iMax,kMax) 
    ...
    REAL(KIND=r8), INTENT(OUT  ) :: liftidx  (nCols)  
    ! Local variables
    ...
    REAL(KIND=r8), DIMENSION(nCols,kMax+1)  :: prsf,tparcel,tenv
    REAL(KIND=r8):: PSFC (nCols)   ! psfc is pressure in hPa
    ...
    REAL(KIND=r8):: e, eth, ethmax, q, dz, cpm,qs,tlclK,rh
    ...
    INTEGER                   :: k850(nCols),k700(nCols),k500(nCols)

  !p-> pressure in mbar
    tparcel=0.0_r8;tenv=0.0_r8
    liftidx=0.0_r8
    delz=0.0_r8
    buoy(0:150)=0.0_r8
    zrel(0:150)=0.0_r8
    ...
    DO i=1,nCols
        PSFC(i)=prsi(i,1)/100.0_r8        ! pressure in hPa
    END DO 
    DO k = 1, kMax
       DO i=1,nCols
            !prs(i,k)      =    PSFC(i)*sl(k)             ! pressure in hPa
            prs(i,k)      =    prsl(i,k)/100.0_r8         ! pressure in hPa
       END DO
    END DO

    CALL INDEXLEVEL(prsl,nCols,kMax,k850,k700,k500)
       ...
       ELSE                           !! 2D case
          ...
          DO k = 1, kMax
             IF ( ght(i,k)-ter(i) .LT. 3000.0_r8 ) THEN
                q        = MIN(MAX(qvp(i,k),1.e-12_r8),0.03_r8)
                temp     = tmk(i,k)
                pressure = prs(i,k)
                e        = MAX((q*pressure)/(EPS+q),1.e-9_r8)
                !PRINT*,temp,e,q,pressure
...
33           kk=kk+1                ! for arrays that go bottom to top
          ...
          qs=SaturationMixingRatio ( tK(i,klcl), prsi(i,klcl) )
          rh=MIN(MAX(qv(i,klcl)/qs,1.e-12_r8),1.0_r8)
          tlclK = TLCLTK (tK(i,klcl),rh)
          liftidx(i)  = tenv(i,k500(i))-tparcel(i,k500(i))! prsi(i,klcl) * ( (tlclK/tK(i,klcl))**(Cp/Rd) )
...
102       CONTINUE
  ...
  !~                                                                          
  !!!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
  REAL(KIND=r8) FUNCTION TLCLTK ( tk, rh )
    
    IMPLICIT NONE
 
    REAL(KIND=r8), INTENT ( IN ) :: tK   !~ Temperature ( K )
    REAL(KIND=r8), INTENT ( IN ) :: rh   !~ Relative Humidity ( % )
!    REAL(KIND=r8)                :: tlcl
    
    REAL(KIND=r8) :: denom, term1, term2

    term1 = 1.0_r8 / ( tK - 55.0_r8 )
    IF ( rh > REAL (0,kind=r8) ) THEN
      term2 = ( LOG (rh/100.0_r8)  / 2840.0_r8 )
    ELSE
      term2 = ( LOG (0.001_r8/1.0_r8) / 2840.0_r8 )
    END IF
    denom = term1 - term2
    TLCLTK = ( 1.0_r8 / denom ) + REAL ( 55,kind=r8 ) 
  ...
  !~
  !!!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
  REAL(KIND=r8) FUNCTION SaturationMixingRatio ( tK, p )! result ( ws )
    ...
    es = 6.122_r8 * exp ( (17.67_r8*(tK-273.15_r8))/ (tK-29.66_r8) )
    SaturationMixingRatio = MIN(MAX(( 0.622_r8*es ) / ( (p/100.0_r8)-es ),1e-12_r8),1.0_r8)
...
!
!-------------------------------------------------------------------------------
  FUNCTION K_index(T0,prsl,nCols,kMax)
    IMPLICIT NONE
    ! k-index   
    ...
    REAL(KIND=r8), INTENT(in) :: prsl(nCols,kMax)  !Pa
    INTEGER                   :: n, i,k850(nCols),k700(nCols),k500(nCols)
    REAL(KIND=r8)             :: K_index(nCols)
    REAL(KIND=r8)             :: pvs700(nCols)
    REAL(KIND=r8)             :: pvs850(nCols)
    REAL(KIND=r8)             :: DewPoint850(nCols)
    REAL(KIND=r8)             :: DewPoint700(nCols)
    ...
    K_index=0.0_r8
    DO i=1, n
      pvs850(i)=fpvsq(T0(i,k850(i)))            !saturation vapor pressure in Pascals
      pvs700(i)=fpvsq(T0(i,k700(i)))            !saturation vapor pressure in Pascals
    ENDDO
    DewPoint850=DewPoint(pvs850,nCols)
    DewPoint700=DewPoint(pvs700,nCols)
    DO i=1, n
      K_index(i) = (T0(i,k850(i)) -  T0(i,k500(i))) + DewPoint850(i) -  (T0(i,k700(i)) -  DewPoint700(i))
    ENDDO
  END FUNCTION K_index
!-------------------------------------------------------------------------------
!-------------------------------------------------------------------------------
  FUNCTION SWEAT_index(T0,U0,V0,prsl,nCols,kMax)
    IMPLICIT NONE
    ! k-index   
    !(http://www.cimms.ou.edu/~schultz/papers/doswell_schultz_indices.pdf george (1960))
    INTEGER      , INTENT(in) :: nCols
    ...
    INTEGER :: k,k850(nCols),k700(nCols),k500(nCols)
    ...
    ENDDO
    DewPoint850=DewPoint(pvs850,nCols)
    TT=TT_index(T0,prsl,nCols,kMax)
    DO i=1, n
       SWEAT_index(i) = 20.0_r8*MAX(((TT(i)) - 49.0_r8),0.0_r8) + 12.0_r8*(DewPoint850(i)-273.16_r8) + &
                         2.0_r8* Vknots(i,k850(i)) + Vknots(i,k500(i)) + 125.0_r8*(sin(DirV(i,k500(i)) - DirV(i,k850(i)) )+ 0.2_r8)
       SWEAT_index(i) = MAX(SWEAT_index(i),0.0_r8)  
    ENDDO
  END FUNCTION SWEAT_index
!-------------------------------------------------------------------------------
  FUNCTION TT_index(T0,prsl,nCols,kMax)
    IMPLICIT NONE
    ! Total Totals  
    ...
    VT=VT_index(T0,prsl,nCols,kMax)
    CT=CT_index(T0,prsl,nCols,kMax)
    TT_index=0.0_r8
    TT_index = (VT + CT)
  END FUNCTION TT_index
!-------------------------------------------------------------------------------

  FUNCTION CT_index(T0,prsl,nCols,kMax)
    IMPLICIT NONE
    ! Cross totals  
    !(http://www.cimms.ou.edu/~schultz/papers/doswell_schultz_indices.pdf Miller (1972))
    INTEGER      , INTENT(in) :: nCols
    ...
    DewPoint850=DewPoint(pvs850,nCols)
    DO i=1, n
      CT_index(i) = (DewPoint850(i) -  T0(i,k500(i)))
    ENDDO
  END FUNCTION CT_index


!-------------------------------------------------------------------------------
  FUNCTION VT_index(T0,prsl,nCols,kMax)
    IMPLICIT NONE
    ! Vertical Totals   
    !(http://www.cimms.ou.edu/~schultz/papers/doswell_schultz_indices.pdf Miller (1972))
    INTEGER      , INTENT(in) :: nCols
    ...
    REAL(KIND=r8), INTENT(in) :: T0(nCols,kMax)
    REAL(KIND=r8), INTENT(in) :: prsl(nCols,kMax)
    INTEGER                   :: i,k850(nCols),k700(nCols),k500(nCols)
    ...
    VT_index=0.0_r8
    DO i=1, nCols
      VT_index(i) = (T0(i,k850(i)) -  T0(i,k500(i)))
    ENDDO
  ...
  FUNCTION liftix_index(T0,Q0,tsfc,prsl,prsi,phil,nCols,kMax)
    IMPLICIT NONE
    INTEGER      , INTENT(in) :: nCols
    INTEGER      , INTENT(in) :: kMax

    REAL(KIND=r8), INTENT(in) :: T0(nCols,kMax)
    REAL(KIND=r8), INTENT(in) :: Q0(nCols,kMax)
    REAL(KIND=r8), INTENT(in) :: tsfc(nCols)
    REAL(KIND=r8), INTENT(in) :: prsl(nCols,kMax)
    REAL(KIND=r8), INTENT(in) :: prsi(nCols,kMax+1)
    REAL(KIND=r8), INTENT(in) :: phil(nCols,kMax)
    REAL(KIND=r8)             :: liftix_index(nCols)

    REAL(KIND=r8) :: ps
    REAL(KIND=r8) :: pt   (kMax+1)
    REAL(KIND=r8) :: tpt  (kMax+1)
    REAL(KIND=r8) :: shpt (kMax+1)
    REAL(KIND=r8) :: pd   (kMax )
    REAL(KIND=r8) :: p    (kMax )
    REAL(KIND=r8) :: t    (kMax )
    REAL(KIND=r8) :: sh   (kMax )
    REAL(KIND=r8) ::  h   (kMax )
    REAL(KIND=r8) :: sli
    REAL(KIND=r8) :: scape
    REAL(KIND=r8) :: scin
    ...
    DO i=1, nCols
       ...
       CALL p2pt( &
                   ...
                   ps               ,&!real   ,intent(in ):: ps
                   p      (1:kMax  ),&!real   ,intent(in ):: p(km)
                   pd     (1:kMax  ),&!real   ,intent(in ):: pd(km)
                   t      (1:kMax  ),&!real   ,intent(in ):: t(km)
                   sh     (1:kMax  ),&!real   ,intent(in ):: sh(km)
                   kMax+1           ,&!integer,intent(in ):: kpt
                   pt     (1:kMax+1),&!real   ,intent(in ):: pt   (kpt)
                   tpt    (1:kMax+1),&!real   ,intent(out):: tpt  (kpt)
                   shpt   (1:kMax+1) )!real   ,intent(out):: shpt (kpt)

!      DO k=1,kMax
!          PRINT*, k,pt (k), tpt  (k), t  (k), sh (k),  shpt(k)
!      END DO
      CALL liftix( &
                 kMax +1          , &!INTEGER      ,INTENT(in ) :: kpt
                 kMax             , &!INTEGER      ,INTENT(in ) :: kMax
                 ps               , &!REAL(KIND=r8),INTENT(in ) :: ps
                 pt   (1:kMax+1)  , &!REAL(KIND=r8),INTENT(in ) :: pt   (kMax+1)
                 tpt  (1:kMax+1)  , &!REAL(KIND=r8),INTENT(in ) :: tpt  (kMax+1)
                 shpt (1:kMax+1)  , &!REAL(KIND=r8),INTENT(in ) :: shpt (kMax+1)
                 p    (1:kMax )   , &!REAL(KIND=r8),INTENT(in ) :: p    (kMax )
                 t    (1:kMax )   , &!REAL(KIND=r8),INTENT(in ) :: t    (kMax )
                 sh   (1:kMax )   , &!REAL(KIND=r8),INTENT(in ) :: sh   (kMax )
                 h    (1:kMax )   , &!REAL(KIND=r8),INTENT(in ) ::  h   (kMax )
                 sli              , &!REAL(KIND=r8),INTENT(out) :: sli
                 scape            , &!REAL(KIND=r8),INTENT(out) :: scape
                 scin             , &!REAL(KIND=r8),INTENT(out) :: scin
                 bli              , &!REAL(KIND=r8),INTENT(out) :: bli
                 bcape            , &!REAL(KIND=r8),INTENT(out) :: bcape
                 bcin               )!REAL(KIND=r8),INTENT(out) :: bcin
      liftix_index(i)=bli
    ENDDO
  END FUNCTION liftix_index
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  subroutine p2pt( &
                   km     ,&!integer,intent(in ):: km
                   ps     ,&!real   ,intent(in ):: ps
                   p      ,&!real   ,intent(in ):: p(km)
                   pd     ,&!real   ,intent(in ):: pd(km)
                   t      ,&!real   ,intent(in ):: t(km)
                   sh     ,&!real   ,intent(in ):: sh(km)
                   kpt    ,&!integer,intent(in ):: kpt
                   pt     ,&!real   ,intent(in ):: pt   (kpt)
                   tpt    ,&!real   ,intent(out):: tpt  (kpt)
                   shpt    )!real   ,intent(out):: shpt (kpt)
!$$$  Subprogram documentation block
!
...
!
! Usage:  call p2pt(km,ps,pd,u,v,t,sh,kpt,pt,tpt,shpt)
!   Input argument list:
...
!$$$
    ...
    real(kind=r8)   ,intent(in ):: p (1:km  )
    real(kind=r8)   ,intent(in ):: pd(km)
    real(kind=r8)   ,intent(in ):: t (km)
    real(kind=r8)   ,intent(in ):: sh(km)
    integer         ,intent(in ):: kpt
    real(kind=r8)   ,intent(in ):: pt   (kpt)
    real(kind=r8)   ,intent(out):: tpt  (kpt)
    real(kind=r8)   ,intent(out):: shpt (kpt)
    real(kind=r8) :: pi (1:kpt)
    real(kind=r8) :: pta(0:kpt)
    real(kind=r8) :: ptd
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    END DO
    pta(kpt)=p(km)/2.0
    call rsearch1(km,pi(1:km),kpt,pta(1:kpt),loc(1:kpt))
    l1=1
    do k=1,kpt
      l2=loc(k)
      tpt(k)=0.0_r8
      shpt(k)=0.0_r8
      do l=l1,l2
        ptd=max(min(pi(l),pta(k-1))-max(pi(l+1),pta(k)),0.0_r8)
        tpt(k)=tpt(k)+ptd*t(l)
  ...
  ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  SUBROUTINE liftix( &
                    kpt   , &!INTEGER      ,INTENT(in ) :: kMax+1
                    kMax  , &!INTEGER      ,INTENT(in ) :: kMax
                    ps    , &!REAL(KIND=r8),INTENT(in ) :: ps
                    pt    , &!REAL(KIND=r8),INTENT(in ) :: pt   (kMax+1)
                    tpt   , &!REAL(KIND=r8),INTENT(in ) :: tpt  (kMax)
                    shpt  , &!REAL(KIND=r8),INTENT(in ) :: shpt (kMax)
                    p     , &!REAL(KIND=r8),INTENT(in ) :: p    (kMax )
                    t     , &!REAL(KIND=r8),INTENT(in ) :: t    (kMax )
                    sh    , &!REAL(KIND=r8),INTENT(in ) :: sh   (kMax )
                    h     , &!REAL(KIND=r8),INTENT(in ) ::  h   (kMax )
                    sli   , &!REAL(KIND=r8),INTENT(out) :: sli
                    scape , &!REAL(KIND=r8),INTENT(out) :: scape
                    scin  , &!REAL(KIND=r8),INTENT(out) :: scin
                    bli   , &!REAL(KIND=r8),INTENT(out) :: bli
    ...
    !   1999-10-18  Mark Iredell
    !
    ! Usage:  call liftix(ps,kpt,pt,tpt,shpt,kMax,p,t,sh,h,&
    !                     sli,scape,scin,bli,bcape,bcin)
    !   Input argument list:
    !     ps       real surface pressure (Pa)
    !     kpt      integer number of pressure layers
    !     pt       real (kpt) pressure layer edges above surface (Pa)
    !     tpt      real (kpt) temperature layer edges above surface  (K)
    !     shpt     real (kpt) specific humidity  layer edges above surface (kg/kg)
    !     kMax       integer number of levels
    !     p        real (kMax) pressure (Pa)
    !     t        real (kMax) temperature (K)
    !     sh       real (kMax) specific humidity (kg/kg)
    !     h        real (kMax) height (m)
    !   Output argument list:
    ...
    REAL(KIND=r8),INTENT(in ) :: ps
    REAL(KIND=r8),INTENT(in ) :: pt   (kpt)
    REAL(KIND=r8),INTENT(in ) :: tpt  (kpt)
    REAL(KIND=r8),INTENT(in ) :: shpt (kpt)
    REAL(KIND=r8),INTENT(in ) :: p    (kMax )
    REAL(KIND=r8),INTENT(in ) :: t    (kMax )
    REAL(KIND=r8),INTENT(in ) :: sh   (kMax )
    REAL(KIND=r8),INTENT(in ) :: h    (kMax )
    REAL(KIND=r8),INTENT(out) :: sli
    REAL(KIND=r8),INTENT(out) :: scape
    REAL(KIND=r8),INTENT(out) :: scin
    REAL(KIND=r8),INTENT(out) :: bli
    REAL(KIND=r8),INTENT(out) :: bcape
    REAL(KIND=r8),INTENT(out) :: bcin
    !    REAL(KIND=r8),parameter:: plift=500.e+2,ptop=40.e+2
    REAL(KIND=r8),PARAMETER:: ptop=40.e+2_r8

    INTEGER k,ki(1)
    REAL(KIND=r8) :: pm,pv,tr,tdpd,tlcl,pklcl,thelcl,pkmas,themas,pkmab,themab
    REAL(KIND=r8) :: tlift,pliftr,pliftk,pks,pkb,tma,shma,pr,pk
    REAL(KIND=r8) :: pta(0:kpt),tvenv,gdz,tvcld,plift(1)
    ...
    bcape=0.0_r8;
    bcin=0.0_r8;
    pm=0.0_r8;pv=0.0_r8;tr=0.0_r8;tdpd=0.0_r8;tlcl=0.0_r8;pklcl=0.0_r8;thelcl=0.0_r8;
    pkmas=0.0_r8;themas=0.0_r8;pkmab=0.0_r8;themab=0.0_r8;
    tlift=0.0_r8;pliftr=0.0_r8;pliftk=0.0_r8;pks=0.0_r8;pkb=0.0_r8;
    tma=0.0_r8;shma=0.0_r8;pr=0.0_r8;pk=0.0_r8;
    pta(0:kMax)=0.0_r8;tvenv=0.0_r8;gdz=0.0_r8;tvcld=0.0_r8;plift(1)=0.0_r8;
    ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
!    pta(kpt)=p(kMax)/2.0
    ...
    DO k=1,kpt
       pm=pt(k)!0.5_r8*(pta(k)+pta(k+1))
       pv=pm*shpt(k)/(con_eps-con_epsm1*shpt(k))     !PK
       tr=tpt(k)                          !PK
       tdpd=MAX(tr-ftdp(pv),0._r8)
       !   Input argument list:
       !     t          Real(r8) LCL temperature in Kelvin
       !     tdpd       Real(r8) dewpoint depression in Kelvin
       !
    ...
    CALL rsearch1(kMax,p(1:kMax),1,plift(1),ki(1))
!    PRINT*,p(ki(1)),ki(1)

    k=ki(1)
    IF(k.GT.0.AND.k.LT.kMax) THEN
       tlift=t(k)!+LOG(p(k)/plift(1))/LOG(p(k)/p(k+1))*(t(k+1)-t(k))
       pliftr=plift(1)
      !   Input argument list:
      !     p          Real(r8) pressure in Pascals
      !
...
!PRINT*,tlift,tma*pliftk/pkb,tma,pliftk,pkb
    ELSE
       sli=0.0_r8
       bli=0.0_r8
    ENDIF
    ...
    DO k=kMax-1,2,-1
       IF(p(k).GT.ptop) THEN
          pr=p(k)
          pk=fpkap(pr)
          tvenv=t(k)*(1.0_r8+con_fvirt*sh(k))
          gdz=con_g*0.5_r8*(h(k+1)-h(k-1))
          IF(pk.LE.pkmas) THEN
             CALL stma(themas,pk,tma,shma)
             tvcld=tma*(1.0_r8+con_fvirt*shma)
             IF(tvcld.GT.tvenv) THEN
                scape=scape+gdz*LOG(tvcld/tvenv)
             ELSEIF(scape.GT.0.0_r8) THEN
          ...
          IF(pk.LE.pkmab) THEN
             ...
             ELSEIF(bcape.GT.0.0_r8) THEN
                bcin=bcin+gdz*LOG(tvcld/tvenv)
             ENDIF
          ENDIF
       ENDIF
    ENDDO
  END SUBROUTINE liftix
...
!
 !-------------------------------------------------------------------------------
  SUBROUTINE rsearch1(km1,z1,km2,z2,l2)
    ...
    REAL(KIND=r8),INTENT(in):: z1(km1),z2(km2)
...
CONTAINS
  ...
  FUNCTION PSIm2(P0,T0,H0,Ustar,Zlev,nCols)
    ...
    DO i=1, n
       ...
       END IF
    ENDDO
!>VBCsep2014: For Eq 11.6 of S Pal Arya (2001) alfa=1. The equations above are using alfa=0.47. RichardNumber2 are based on PSIm2.
  END FUNCTION PSIm2
 ...
 !>VBCsep14
 FUNCTION QSIm2_mod(P0,T0,H0,Ustar,Zlev,nCols)
  USE Constants, ONLY : pai
    !--------------------------------------------------------------------
    ! Simularity function for wind  Profile                           !
    !(equation 11.14 Arya P.S. Introduction to Micrometeorology)      !
    ! Improvements in the unstable similarity equation                  !
    !--------------------------------------------------------------------
    IMPLICIT NONE
    INTEGER      , INTENT(IN) :: nCols
    REAL(KIND=r8), INTENT(in) :: P0(nCols)
    ...
    REAL(KIND=r8), INTENT(IN) :: ZLev
    INTEGER                   :: n, i
    REAL(KIND=r8)             :: QSIm2_mod(SIZE(P0))
    REAL(KIND=r8)             :: xx   (SIZE(P0))
    REAL(KIND=r8)             :: boyance_parameter(SIZE(P0))
    n=SIZE(P0)
    boyance_parameter = zLev/ScaleLength2(P0,T0,H0,Ustar,nCols)
    ...
    DO i=1, n
       IF(boyance_parameter(i)< 0.0_r8)THEN
          xx(i)=(1.0_r8 -15.0_r8*boyance_parameter(i))**(1.0_r8/4.0_r8)
          QSIm2_mod(i)=LOG(((1.0_r8 + xx(i)**2.0_r8)/2.0_r8)*(((1.0_r8+xx(i))/2.0_r8)**2.0_r8))& ! unstable
                     -2.0_r8*ATAN(xx(i))+pai/2.0_r8
       ELSE
          QSIm2_mod(i) = -5.0_r8*boyance_parameter(i)                             ! stable
       END IF
    ENDDO
  END FUNCTION QSIm2_mod


  FUNCTION QSIm2_j(P0,T0,H0,Ustar,Zlev,nCols)
    !--------------------------------------------------------------------------------------------
    ! Similarity function for momentum                                            !
    !                                                                        !
    ! Jimenez,P A;Duhia, J; Gonzalez-Rouco,J F; Navarro, J; Montavez, J P; Garcia-Bustamente, E.!
    ! A revised scheme for the WRF Surface Layer formulation. Monthly Weather Review. 2012.      !
    !                                                                        !
    ! Fairall, C. W., E. F. Bradley, D. P. Rogers, J. B. Edson, and G. S. Young, 1996: Bulk      !
    ! parametrization of air-sea fluxes for Tropical Ocean Global Atmosphere Coupled-Ocean      !
    ! Atmosphere Response Experiment. J. Geophys. Res., 101 (C2), 3747-3764.                   !
    !                                                                        !
    ! Cheng, Y. and Burtsaert, 2005: Flux-profile relationships for wind speed and temperature      !
    ! in the stable atmospheric boundary layer. Bound.-Layer Meteor., 114,519-538.            !
    !--------------------------------------------------------------------------------------------
    USE Constants, ONLY : pai
    IMPLICIT NONE
    INTEGER      , INTENT(IN) :: nCols
    ...
    REAL(KIND=r8), INTENT(IN) :: ZLev
    INTEGER                   :: n, i
    REAL(KIND=r8)             :: QSIm2_j(SIZE(P0))
    REAL(KIND=r8)             :: x(SIZE(P0))
    REAL(KIND=r8)             :: y(SIZE(P0))
    REAL(KIND=r8)             :: boyance_parameter(SIZE(P0))
    REAL(r8)                  :: QSIKm(SIZE(P0)) !Kansas-type function (Paulson 1970, Arya 1988)
    REAL(r8)                  :: QSICm(SIZE(P0)) !Convective contribution
    REAL(r8)                  :: am=10.0_r8
    REAL(r8)                  :: a=6.1_r8
    REAL(r8)                  :: b=2.5_r8
    n=SIZE(P0)
    ...
    QSIm2_j=0.0_r8
    DO i=1, n
       IF(boyance_parameter(i)< 0.0_r8)THEN !Unstable conditions (Fairall et al 1996)
          x(i)=(1.0_r8 -15.0_r8*boyance_parameter(i))**(1.0_r8/4.0_r8)
          QSIKm(i)=LOG(((1.0_r8 + x(i)**2.0_r8)/2.0_r8)*(((1.0_r8 + x(i))/2.0_r8)**2.0_r8))&
                     -2.0_r8*ATAN(x(i))+pai/2.0_r8 
          y(i) = (1.0_r8-am*boyance_parameter(i))**(1.0_r8/3.0_r8)
          QSICm(i)=(3.0_r8/2.0_r8)*LOG(y(i)**2.0_r8+y(i)+1.0_r8/3.0_r8)-SQRT(3.0_r8)       &
                     *ATAN(2.0_r8*y(i)+1.0_r8/SQRT(3.0_r8))+pai/SQRT(3.0_r8)
          QSIm2_j(i)=(QSIKm(i)+(boyance_parameter(i)**2.0_r8)*QSICm(i))                      &
                     /(1.0_r8+boyance_parameter(i)**2.0_r8)
       ELSE                                 !Stable conditions (Cheng and Burtsaert 2005)
          QSIm2_j(i) = -a*LOG(boyance_parameter(i)+(1.0_r8+boyance_parameter(i)**b)**(1.0_r8/b))
       END IF
    ENDDO
  END FUNCTION QSIm2_j



  FUNCTION QSIh2_j(P0,T0,H0,Ustar,Zlev,nCols)
    !--------------------------------------------------------------------------------------------
    ! Similarity function for temperature and humidity                                     !
    !                                                                        !
    ! Jimenez,P A;Duhia, J; Gonzalez-Rouco,J F; Navarro, J; Montavez, J P; Garcia-Bustamente, E.!
    ! A revised scheme for the WRF Surface Layer formulation. Monthly Weather Review. 2012.      !
    !                                                                        !
    ...
    REAL(KIND=r8), INTENT(IN) :: ZLev
    INTEGER                   :: n, i
    REAL(KIND=r8)             :: QSIh2_j(SIZE(P0))
    REAL(KIND=r8)             :: x(SIZE(P0))
    REAL(KIND=r8)             :: y(SIZE(P0))
    REAL(KIND=r8)             :: boyance_parameter(SIZE(P0))
    REAL(r8)                  :: QSIKh(SIZE(P0)) !Kansas-type function (Paulson 1970, Arya 1988)
    REAL(r8)                  :: QSICh(SIZE(P0)) !Convective contribution
    REAL(r8)                  :: ah=34.0_r8
    REAL(r8)                  :: c=5.3_r8
    REAL(r8)                  :: d=1.1_r8
    n=SIZE(P0)
    ...
    DO i=1, n
       IF(boyance_parameter(i)< 0.0_r8)THEN !Unstable conditions (Fairall et al 1996)
          x(i)=(1.0_r8 -15.0_r8*boyance_parameter(i))**(1.0_r8/4.0_r8)
          QSIKh(i)=LOG(((1.0_r8 + x(i)**2.0_r8)/2.0_r8)*(((1.0_r8 + x(i))/2.0_r8)**2.0_r8))&
                     -2.0_r8*ATAN(x(i))+pai/2.0_r8 
          y(i) = (1.0_r8-ah*boyance_parameter(i))**(1.0_r8/3.0_r8)
          QSICh(i)=(3.0_r8/2.0_r8)*LOG(y(i)**2.0_r8+y(i)+1.0_r8/3.0_r8)-SQRT(3.0_r8)       &
                     *ATAN(2.0_r8*y(i)+1.0_r8/SQRT(3.0_r8))+pai/SQRT(3.0_r8)
          QSIh2_j(i)=(QSIKh(i)+(boyance_parameter(i)**2.0_r8)*QSICh(i))                    &
                     /(1.0_r8+boyance_parameter(i)**2.0_r8)
       ELSE                                 !Stable conditions (Cheng and Burtsaert 2005)
          QSIh2_j(i) = -c*LOG(boyance_parameter(i)+(1.0_r8+boyance_parameter(i)**d)**(1.0_r8/d))
       END IF
    ENDDO
  END FUNCTION QSIh2_j


  FUNCTION T2MT(P0,T0,H0,Ustar,Zlev,z0,ztn,tsurf,gt,nCols,msk)
    !--------------------------------------------------------------------------------------------
    ! Temperature Profile Method                                                  !
    ! Option sflyr=3 --> Eq 11.27                                                      !
    ! Arya P.S. Introduction to Micrometeorology (1988)                                    !
    !                                                                        !
    ! Option sflyr=1 --> pg 901                                                      !
    ! Option sflyr=2 --> Eq 25                                                      !
    ! Jimenez,P A;Duhia, J; Gonzalez-Rouco,J F; Navarro, J; Montavez, J P; Garcia-Bustamente, E.!
    ! A revised scheme for the WRF Surface Layer formulation. Monthly Weather Review. 2012.       !
    !                                                                                           !
    ! Option sflyr=3                                                            !
    ! ECMWF Physical Process (pg 51)                                                !
    ! site: http://old.ecmwf.int/research/ifsdocs/CY40r1/IFSPart4.pdf                         !
    !--------------------------------------------------------------------------------------------
    USE Options, ONLY : sflyr
    IMPLICIT NONE
    INTEGER      , INTENT(IN) :: nCols
    REAL(KIND=r8), INTENT(in) :: P0(nCols)
    REAL(KIND=r8), INTENT(in) :: T0(nCols)
    ...
    REAL(KIND=r8)             :: psi0   (SIZE(P0))
    REAL(KIND=r8)             :: psi1   (SIZE(P0))
    REAL(KIND=r8)             :: psi2   (SIZE(P0))
    REAL(KIND=r8)             :: psi3   (SIZE(P0))
    REAL(KIND=r8)             :: psi4   (SIZE(P0))
    REAL(KIND=r8)             :: psi5   (SIZE(P0))
    REAL(KIND=r8)             :: sensi  (SIZE(P0))
    REAL(KIND=r8)             :: ustar2 (SIZE(P0))
    REAL(KIND=r8)             :: zrough (SIZE(P0))
    REAL(KIND=r8)             :: ztn    (SIZE(P0))
    REAL(KIND=r8)             :: tsurf  (SIZE(P0))
    REAL(KIND=r8)             :: gt     (SIZE(P0))
    REAL(KIND=r8)             :: rho
    REAL(KIND=r8)             :: tstar
    INTEGER                   :: sfo
    REAL(KIND=r8)             :: aa, bb
    ...
    sfo=1 !Similarity function option
    T2MT=0.0_r8
    DO i=1,n
         zLev2=ztn(i)
         tsig =gt(i)
        IF (sfo==1) THEN
           psi0 =  QSIh2(P0,T0,sensi,ustar2,zLev,nCols)
           psi1 =  QSIh2(P0,T0,sensi,ustar2,zLev2,nCols)
           psi2 =  QSIh2(P0,T0,sensi,ustar2,zLev,nCols)
           psi3 =  QSIh2(P0,T0,sensi,ustar2,zLev2+z0(i),nCols)
           psi4 =  QSIh2(P0,T0,sensi,ustar2,zLev +z0(i),nCols)
           psi5 =  QSIh2(P0,T0,sensi,ustar2,z0(i),nCols)
        ELSE IF (sfo==2) THEN
           psi0 =  QSIh2_j(P0,T0,sensi,ustar2,zLev,nCols)
           psi1 =  QSIh2_j(P0,T0,sensi,ustar2,zLev2,nCols)
           psi2 =  QSIh2_j(P0,T0,sensi,ustar2,zLev,nCols)
           psi3 =  QSIh2_j(P0,T0,sensi,ustar2,zLev2+z0(i),nCols)
           psi4 =  QSIh2_j(P0,T0,sensi,ustar2,zLev+z0(i),nCols)
           psi5 =  QSIh2_j(P0,T0,sensi,ustar2,z0(i),nCols)
        ENDIF
      IF (sflyr==1) THEN !Eq 11.17 of P. Arya
       rho    = P0(i)/(cts_gases*T0(i))
       tstar  = -sensi(i)/(rho*cts_heatespecific*ustar2(i))
       T2MT(i) = (tstar/cts_vonKarman) * ( LOG(Zlev) -  psi0(i)) &
            + (T0(i) -  (tstar/cts_vonKarman)*LOG(z0(i)))
       T2MT(i) = MIN(MAX(T2MT(i),T0(i)*0.97_r8),T0(i)*1.03_r8)
      ELSE IF (sflyr==2 .or. sflyr==3) THEN !Eq 25 of Jimenez et al (2012) or ECMWF documentation
         aa= ( LOG(zLev +z0(i)) - LOG(z0(i)) - psi4(i) + psi5(i))
         bb= ( LOG(Zlev2+z0(i)) - LOG(z0(i)) - psi3(i) + psi5(i))
         IF(msk ==1)THEN
             T2MT(i) =tsig+(tsig-tsurf(i))*(aa/bb)
         ELSE
             T2MT(i) =tsurf(i)+(tsig-tsurf(i))*(aa/bb)
         END IF
!         T2MT(i)d =T0(i)+(tsig-T0(i))*(aa/bb)
!         T2MT(i) = MIN(MAX(T2MT(i),T0(i)*0.97_r8),T0(i)*1.03_r8)
         T2MT(i) = MIN(MAX(T2MT(i),tsig*0.90_r8),tsig*1.10_r8) !as Dr. Silvio suggested.
      END IF
  ...
  FUNCTION Q2MT(P0,T0,Q0,H0,E0,Ustar,Zlev,z0,ztn,qsurf,gq,zl,nCols)
    !--------------------------------------------------------------------------------------------
    ! Humidity Profile Method                                                        !
    ! Option sflyr=4 --> Eq 11.27                                                      !
    ! Arya P.S. Introduction to Micrometeorology (1988)                                    !
    !                                                                        !
    ! Option sflyr=1 --> pg 901                                                      !
    ! Option sflyr=2 --> Eq 26                                                      !
    ! Jimenez,P A;Duhia, J; Gonzalez-Rouco,J F; Navarro, J; Montavez, J P; Garcia-Bustamente, E.!
    ! A revised scheme for the WRF Surface Layer formulation. Monthly Weather Review. 2012.      !
    !                                                                        !
    ! Option sflyr=3                                                            !
    ...
    REAL(KIND=r8), INTENT(in) :: Ustar(nCols)
    REAL(KIND=r8), INTENT(in) :: z0(nCols)
    REAL(KIND=r8), INTENT(IN) :: zl (nCols) !viscous sublayer height (zl=0.01 for land and zl=Z0 for ocean))
    REAL(KIND=r8), INTENT(IN) :: ZLev
    INTEGER                   :: n, i
    ...
    REAL(KIND=r8)             :: psi5   (SIZE(P0))
    REAL(KIND=r8)             :: ztn    (SIZE(P0))
    REAL(KIND=r8)             :: qsurf  (SIZE(P0))
    REAL(KIND=r8)             :: gq     (SIZE(P0))
    REAL(KIND=r8)             :: sensi  (SIZE(P0))
    REAL(KIND=r8)             :: ustar2 (SIZE(P0))
    REAL(KIND=r8)             :: zrough (SIZE(P0))
    REAL(KIND=r8)             :: rho
    ...
    REAL(KIND=r8)             :: zLev2
    REAL(KIND=r8)             :: qsig
    REAL(KIND=r8)             :: cp = 1004.6_r8! specific heat of air   (j/kg/k)
    REAL(KIND=r8)             :: cs = 0.000024_r8 !cs is the effective heat 
                                               !transfer coefficient for
                                               !nonturbulent processest(molecular diffusivity)
                                               !Grell et al (1994) pg 82, Eq. 5.4.3.15
    ...
    DO i=1,n
       ...
       qstar  = -E0(i)/(rho*cts_heatevap*ustar2(i))
        IF (sfo==1) THEN
          IF (sflyr==2 .OR. sflyr==1) THEN
           psi0 =  QSIh2(P0,T0,sensi,ustar2,Zlev,nCols)
           psi1 =  QSIh2(P0,T0,sensi,ustar2,Zlev2,nCols)
           psi2 =  QSIh2(P0,T0,sensi,ustar2,zLev,nCols)
           psi3 =  QSIh2(P0,T0,sensi,ustar2,Zlev2+zl(i),nCols)
           psi4 =  QSIh2(P0,T0,sensi,ustar2,zLev+zl(i),nCols)
           psi5 =  QSIh2(P0,T0,sensi,ustar2,zl(i),nCols)
          ELSE
           psi0 =  QSIh2(P0,T0,sensi,ustar2,Zlev,nCols)
           psi1 =  QSIh2(P0,T0,sensi,ustar2,Zlev2,nCols)
           psi2 =  QSIh2(P0,T0,sensi,ustar2,zLev,nCols)
        ...
        ELSE IF (sfo==2) THEN
          IF (sflyr==2 .OR. sflyr==1) THEN
           ...
           psi1 =  QSIh2_j(P0,T0,sensi,ustar2,Zlev2,nCols)
           psi2 =  QSIh2_j(P0,T0,sensi,ustar2,zLev,nCols)
           psi3 =  QSIh2_j(P0,T0,sensi,ustar2,Zlev2+zl(i),nCols)
           psi4 =  QSIh2_j(P0,T0,sensi,ustar2,zLev+zl(i),nCols)
           psi5 =  QSIh2_j(P0,T0,sensi,ustar2,zl(i),nCols)
          ELSE
           psi0 =  QSIh2_j(P0,T0,sensi,ustar2,Zlev,nCols)
           psi1 =  QSIh2_j(P0,T0,sensi,ustar2,Zlev2,nCols)
           psi2 =  QSIh2_j(P0,T0,sensi,ustar2,zLev,nCols)
        ...
        END IF
      IF (sflyr==1) THEN !Eq 11.17 of P. Arya
       Q2MT(i) = (qstar/cts_vonKarman) * ( LOG(Zlev) -  psi0(i)) &
            + (Q0(i) -  (qstar/cts_vonKarman)*LOG(zrough(i)))
       Q2MT(i) = MIN(MAX(Q2MT(i),Q0(i)*0.5_r8),Q0(i)*1.5_r8)
      ELSE IF (sflyr==2) THEN !Eq 25 of Jimenez et al (2012)
         aa= LOG(((rho*cp*cts_vonKarman*Ustar(i)*zLev)/cs)+(zLev/zl(i)))&
             - psi2(i) +psi5(i)
         bb= LOG(((rho*cp*cts_vonKarman*Ustar(i)*zLev2)/cs) +(zLev/zl(i)))&
             - psi1(i) +psi5(i)
         Q2MT(i) =Q0(i)+(qsig-Q0(i))*(aa/bb)
!         Q2MT(i) =qsurf(i)+(qsig-qsurf(i))*(aa/bb)
         Q2MT(i) = MIN(MAX(Q2MT(i),Q0(i)*0.5_r8),Q0(i)*1.5_r8)  ! original limits
      ELSE IF (sflyr==3) THEN ! ECMWF similarity between t02m and q02m
         aa= ( LOG(zLev+z0(i)) - LOG(z0(i))- psi4(i) +psi5(i))
         bb= ( LOG(Zlev2+z0(i)) - LOG(z0(i))- psi3(i)+psi5(i))
         Q2MT(i) =Q0(i)+(qsig-Q0(i))*(aa/bb)
!         Q2MT(i) =qsurf(i)+(qsig-qsurf(i))*(aa/bb)
         Q2MT(i) = MIN(MAX(Q2MT(i),Q0(i)*0.5_r8),Q0(i)*1.5_r8)  ! original limits
!         Q2MT(i) = MIN(MAX(Q2MT(i),qsurf(i)*0.5_r8),qsurf(i)*1.5_r8) 
      END IF
    END DO
  ...
  FUNCTION W2MT(P0,T0,Speedm,H0,Ustar,Zlev,z0,ztn,nCols,MaskLand)
    !--------------------------------------------------------------------------------------------
    ! Wind Profile Method                                                        !
    ! Option sflyr=1 --> Eq 11.27                                                !
    ! Arya P.S. Introduction to Micrometeorology (1988)                                    !
    !                                                                        !
    ! Option sflyr=2 --> Eq 24                                                      !
    ! Jimenez,P A;Duhia, J; Gonzalez-Rouco,J F; Navarro, J; Montavez, J P; Garcia-Bustamente, E.!
    ! A revised scheme for the WRF Surface Layer formulation. Monthly Weather Review. 2012.      !
    !                                                             !
    ! Option sflyr=3 --> ECMWF                                                      !
    ! ECMWF Physical Process (pg 51)                                                !
    ! site: http://old.ecmwf.int/research/ifsdocs/CY40r1/IFSPart4.pdf                         !
    !-------------------------------------------------------------------------------------------!
    USE Options, ONLY : sflyr
    ...
    DO i=1,n
         zLev2=ztn(i)
         IF (sfo==1) THEN
           psi0 =  QSIm2(P0,T0,sensi,ustar2,Zlev,nCols)
           psi1 =  QSIm2(P0,T0,sensi,ustar2,Zlev2,nCols)
           psi2 =  QSIm2(P0,T0,sensi,ustar2,zLev,nCols)
           psi3 =  QSIm2(P0,T0,sensi,ustar2,Zlev2+z0(i),nCols)
           psi4 =  QSIm2(P0,T0,sensi,ustar2,zLev+z0(i),nCols)
           psi5 =  QSIm2(P0,T0,sensi,ustar2,z0(i),nCols)
         ELSE IF (sfo==2) THEN
           psi0 =  QSIm2_mod(P0,T0,sensi,ustar2,Zlev,nCols)
           psi1 =  QSIm2_mod(P0,T0,sensi,ustar2,Zlev2,nCols)
           psi2 =  QSIm2_mod(P0,T0,sensi,ustar2,zLev,nCols)
           psi3 =  QSIm2_mod(P0,T0,sensi,ustar2,Zlev2+z0(i),nCols)
           psi4 =  QSIm2_mod(P0,T0,sensi,ustar2,zLev+z0(i),nCols)
           psi5 =  QSIm2_mod(P0,T0,sensi,ustar2,z0(i),nCols)
         ELSE IF (sfo==3) THEN
           psi0 =  QSIm2_j(P0,T0,sensi,ustar2,Zlev,nCols)
           psi1 =  QSIm2_j(P0,T0,sensi,ustar2,Zlev2,nCols)
           psi2 =  QSIm2_j(P0,T0,sensi,ustar2,zLev,nCols)
           psi3 =  QSIm2_j(P0,T0,sensi,ustar2,Zlev2+z0(i),nCols)
           psi4 =  QSIm2_j(P0,T0,sensi,ustar2,zLev+z0(i),nCols)
           psi5 =  QSIm2_j(P0,T0,sensi,ustar2,z0(i),nCols)
         END IF

      IF (sflyr==1) THEN ! THEN !Eq 11.17 of P. Arya
         IF(MaskLand == 1)THEN
           W2MT(i) = (Ustar(i)/cts_vonKarman) * ( LOG(Zlev) -  psi0(i)) &
      ...
      ELSE IF (sflyr==2 .or. sflyr==3) THEN !Eq 24 of Jimenez et al (2012) or ECMWF documentation
         aa= (LOG(zLev+z0(i))-LOG(z0(i))-psi4(i)+psi5(i))
         bb= (LOG(Zlev2+z0(i))-LOG(z0(i))-psi3(i)+psi5(i))
         W2MT(i) =Speedm(i)*(aa/bb)
         W2MT(i) = MIN(MAX(W2MT(i),Speedm(i)*0.1_r8),Speedm(i)*1.5_r8)
      END IF
    END DO
  END FUNCTION W2MT
...
MODULE Mod_GET_PRS
  ...
  PUBLIC :: GET_PRS,GET_PHI,sig2press
CONTAINS
  !-------------------------------------------------------------------------------
      ...
      REAL(kind=r8), PARAMETER :: PT01=0.01 !1/100  cb
      REAL(kind=r8), PARAMETER:: con_rd     =2.8705e+2      ! gas constant air    (J/kg/K)
      REAL(kind=r8), PARAMETER:: con_cp     =1.0046e+3      ! spec heat air @p    (J/kg/K)
      REAL(kind=r8), PARAMETER:: con_rocp   =con_rd/con_cp
  ...
  SUBROUTINE GET_PHI(&
    ...
    REAL(kind=r8), INTENT(OUT  ) :: phii(ix,levs+1)     !===>  PHIH(K+1)  INPUT   GEOPOTENTIAL @ EDGES  IN MKS units (m^2/s^2)
    REAL(kind=r8), INTENT(OUT  ) :: phil(ix,levs)       !===>  PHIL(K)    INPUT   GEOPOTENTIAL @ LAYERS IN MKS units (m^2/s^2)
    REAL(kind=r8), INTENT(OUT  ) :: del (ix,levs)
    ...
    ELSE                                   ! Not gc Virt Temp (Orig Joe)
       DO k=1,levs
          DO i=1,ix
             TEM         = con_cp * T(i,k) * (1.0 + con_fvirt*MAX(Q(i,k,1),zero))/ PRKL(i,k)
             DPHIB       = (PRKI(i,k) - PRKL(i,k)) * TEM
             DPHIT       = (PRKL(i,k) - PRKI(i,k+1)) * TEM
             phil(i,k)   = phii(i,k) + DPHIB
             phii(i,k+1) = phil(i,k) + DPHIT
