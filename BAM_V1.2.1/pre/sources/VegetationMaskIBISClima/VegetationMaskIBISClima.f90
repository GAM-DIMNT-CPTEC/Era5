...
!  $Revision: 1.1.1.1 $
!
PROGRAM VegetationMaskIBISClima
   ...
   CHARACTER (LEN=59) :: VarName='VegetationMaskIBISClima'

   CHARACTER (LEN=50) :: VarNameG='VegetationMaskIBISClimaG'

   CHARACTER (LEN=51) :: FileBCs='ibismsk.form'

   CHARACTER (LEN=12) :: DirBCs='pre/databcs/'

   CHARACTER (LEN=52) :: NameNML='VegetationMaskIBISClima.nml'
   ...
   REAL (KIND=r4), DIMENSION (:,:), ALLOCATABLE :: VegMaskGad,VegetationMask2
   ...
   ALLOCATE (VegetationMask(Idim,Jdim), VegetationMask2(Idim,Jdim),VegMaskGad(Idim,Jdim))
   ...
   IF (GrADS) THEN
      ...
      WRITE (UNIT=nfctl, FMT='(A)') '*'
      WRITE (UNIT=nfctl, FMT='(A)') 'TITLE IBIS Vegetation Mask'
      WRITE (UNIT=nfctl, FMT='(A)') '*'
   ...
   PRINT *, "*** VegetationMaskIBISClima ENDS NORMALLY ***"
...
END PROGRAM VegetationMaskIBISClima
